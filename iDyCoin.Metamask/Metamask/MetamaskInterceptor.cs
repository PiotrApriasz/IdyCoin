...
public class MetamaskInterceptor : RequestInterceptor
{
        ...
        string? route = null)
    {
        if (request.Method == "eth_sendTransaction")
        {
            ...
            var response = await _metamaskInterop.SendAsync(new MetamaskRpcRequestMessage(request.Id, request.Method, _metamaskHostProvider.SelectedAccount,
        ...
        string route = null, params object[] paramList)
    {
        if (method == "eth_sendTransaction")
        {
            ...
            var response = await _metamaskInterop.SendAsync(new MetamaskRpcRequestMessage(route, method, _metamaskHostProvider.SelectedAccount, 
                paramList));
            return ConvertResponse<T>(response);
        }
        else
        {
            var response = await _metamaskInterop.SendAsync(new RpcRequestMessage(route, _metamaskHostProvider.SelectedAccount, method,
                paramList));
            return ConvertResponse<T>(response);
        }
